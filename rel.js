...
function start_me_up(){
    ...
    }).then(function(rsp) { return rsp.json() }).then(function(search) {
    ...
    await fetch('https://api.familysearch.org/platform/tree/persons/'+id+'/relationships/'+key.pid, { headers: { Authorization: 'Bearer '+token }}).then(function(rsp) {
    // Handle no relationship case
    if (rsp.status == 204) return { persons: [] };
    return rsp.json();
    })
    .then(function(rsp) {
    ...
    // Get user session
    fetch('https://strategicsolutions.herokuapp.com/tree/api/fs')
    .then(function(rsp) { return rsp.json() })
    .then(function(auth) { token2 = auth.token });
    fill()
}
